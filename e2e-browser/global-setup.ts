...
// (vite build) embedded into the real hub binary via `-tags portal` (see
// hub/internal/portal/portal.go and hub/Makefile's `portal` +
// `build-portal` targets). Every test then boots that exact binary — no
// mocked backend, no dev server proxying to a mocked backend.
//
// Why the embedded build and not `vite dev` pointed at the hub: the
// embedded SPA is what actually ships (the hub binary embeds it and
...
// matters when the frontend and the hub are actually the same origin,
...
const EMBED_DIR = path.join(HUB_DIR, 'internal', 'portal', 'dist');
...
export default async function globalSetup(): Promise<void> {
  ...
  // real hub/internal/portal/dist path — go:embed requires the directory
  // to live inside the module, there's no way to point it elsewhere without
  // editing hub source (out of scope). This path is gitignored (matches
  ...
  const binPath = path.join(scratch, process.platform === 'win32' ? 'aql-hub-e2e.exe' : 'aql-hub-e2e');
  console.log('[e2e-browser] building the hub (-tags portal) ->', binPath);
  ...
  // even though hub/internal/portal/dist on disk had genuinely fresh
  ...
  execFileSync(
    'go',
    ['build', '-a', '-tags', 'portal', '-o', binPath, './cmd/hub'],
    { cwd: HUB_DIR, stdio: 'inherit', env: { ...process.env, CGO_ENABLED: '0' } },
  );
  ...
  process.env.AQL_E2E_HUB_BIN = binPath;
  process.env.AQL_E2E_BUILD_SCRATCH = scratch;
}
