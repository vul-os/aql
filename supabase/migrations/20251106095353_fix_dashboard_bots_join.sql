-- Fix dashboard analytics to properly join bots through locations table
-- Bots have location_id, not organization_id
...
BEGIN
  WITH 
  -- Bot statistics (FIXED: join through locations)
  bot_stats AS (
    SELECT 
      COUNT(*) as total_bots,
      COUNT(*) FILTER (WHERE b.status = 'operational') as operational_bots,
      ...
      COUNT(*) FILTER (WHERE b.status IN ('operational', 'active')) as active_now
    FROM bots b
...
COMMENT ON FUNCTION get_dashboard_analytics_v2(UUID) IS 'Returns comprehensive dashboard analytics for an organization (fixed bots join through locations)';
