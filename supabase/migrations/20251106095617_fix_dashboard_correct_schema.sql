-- Fix dashboard analytics to use correct column names from actual schema
-- Services table has: status (not stage), no completed_at/started_at/area_sqm
...
COMMENT ON FUNCTION get_dashboard_analytics_v2(UUID) IS 'Returns comprehensive dashboard analytics using correct schema (status not stage, no completion tracking yet)';
