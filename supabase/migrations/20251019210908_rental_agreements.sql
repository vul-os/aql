...
CREATE TABLE rental_agreements (
    ...
    number_of_bots INTEGER NOT NULL DEFAULT 1 CHECK (number_of_bots = 1),
...
CREATE INDEX IF NOT EXISTS idx_rental_agreements_pool ON rental_agreements(pool_id);
...
-- Add foreign key constraint to discount_usage table (created in earlier migration)
-- This links discount usage to rental agreements
ALTER TABLE discount_usage
ADD CONSTRAINT fk_discount_usage_rental_agreement
FOREIGN KEY (rental_agreement_id) REFERENCES rental_agreements(id) ON DELETE SET NULL;

-- Comments
COMMENT ON TABLE rental_agreements IS 'Flexible rental agreements - no long-term contracts, pause/cancel anytime. Each agreement covers exactly ONE bot.';
...
COMMENT ON COLUMN rental_agreements.number_of_bots IS 'Always 1 - each rental agreement covers exactly one bot';
