...
type Config struct {
	...
	// WhatsAppEngine is the raw AQL_WHATSAPP_ENGINE value — "cloud"
...
// behavioral spec: WHATSAPP_*, SLACK_*, TELEGRAM_*, no AQL_ prefix — kept
...
// the WhatsApp engine selection and bridge config — uses the AQL_* prefix,
// consistent with the gateway's own infra config (AQL_DATA_DIR,
// AQL_LISTEN, …).
func FromEnv(getenv func(string) string, publicURL string) Config {
	return Config{
		...
		WhatsAppEngine:         getenv("AQL_WHATSAPP_ENGINE"),
		WhatsAppBridgeURL:      getenv("AQL_WHATSAPP_BRIDGE_URL"),
		WhatsAppBridgeAPIKey:   getenv("AQL_WHATSAPP_BRIDGE_API_KEY"),
		WhatsAppBridgeInstance: getenv("AQL_WHATSAPP_BRIDGE_INSTANCE"),
