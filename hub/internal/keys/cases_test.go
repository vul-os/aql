...
//
// controller/internal/jcs is the other one. Go's internal/ rule forbids any single
...
func loadHubJCSCases(t *testing.T) hubJCSCases {
	t.Helper()
...
func TestHubJCSSharedCases(t *testing.T) {
	c := loadHubJCSCases(t)
	for _, tc := range c.Cases {
		t.Run(tc.Name, func(t *testing.T) {
			got, err := keys.CanonicalizeJSON([]byte(tc.Input))
			if err != nil {
	...
	for _, tc := range c.Refused {
		t.Run("refused/"+tc.Name, func(t *testing.T) {
			if got, err := keys.CanonicalizeJSON([]byte(tc.Input)); err == nil {
