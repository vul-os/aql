...
**Shipped, and it replaced the plan this section used to describe:** a device
credential in `-device-config` may be a REFERENCE — `${env:NAME}` or
`${file:/path}` — instead of a value, at the MQTT password, ONVIF camera
passwords and HTTP device headers. So the config file can be version-controlled
and shipped around with no secret in it, and the secret arrives as a Docker
secret, a systemd `LoadCredential=`, a Kubernetes mount or an environment
variable. An unresolvable reference refuses the whole file rather than becoming
an empty credential, because a broker that accepts anonymous connections would
take one.

Be precise about what that does and does not buy. It removes the secret from a
file people copy; it does NOT protect it from anyone who can read the referenced
file or the hub's environment, and the hub process holds it in memory either
way. It is a smaller claim than the one it replaced, and it is a true one.

**This section previously said those credentials were "intended to live in the
OS keychain".** That is withdrawn, on grounds of deployment rather than taste: a
container has no keychain, no logged-in session and no Secret Service on the
bus, `docker run` is a documented install path, and the binary is built
`CGO_ENABLED=0` so it cannot bind macOS Security.framework where one does exist.
A keychain would have worked on a developer's laptop and nowhere a hub runs. See
ROADMAP for the full reasoning.

**Shipped, for the signing key:** `AQL_DATA_KEY` seals `gateway_ed25519.seed`
with AES-256-GCM. It protects the data directory once it has LEFT the running
host — the `tar czf` above, a snapshot, a stolen disk — and it does NOT protect
against anyone who can read the hub's environment or the key file it points at,
because the hub must decrypt unattended. No passphrase prompt, deliberately:
this software opens gates, and a hub that will not restart after a power cut
until somebody drives out to it is a worse failure than a key root can read.

A sealed seed with no data key REFUSES to start. That refusal is part of the
feature rather than a nicety on it: encryption adds a way to lose the hub's
identity, and without the refusal a sealed file fails hex-decoding, reads as
corrupt, and can reach the branch that mints a replacement — which orphans every
paired controller. `verify-restore` reports a sealed key with no `AQL_DATA_KEY`
present, so that is found from the backup rather than at a gate.

The JWT secret is sealed by the same key, and the reason is worth stating
because its two failure axes point opposite ways. LOSING it costs nothing —
sessions end and people sign in again, which is why `verify-restore` calls its
absence harmless. LEAKING it is serious: it signs every session token, so anyone
holding it can mint one for any user, including an admin. A stolen backup is
precisely where the second matters and the first does not.

A sealed secret with no data key refuses too, rather than regenerating.
Regenerating would be harmless in itself and would turn "you forgot
`AQL_DATA_KEY`" into "everyone was logged out and nobody knows why".

**Target:** nothing in the data directory is now unencrypted except the database
itself. Encrypting `lintel.db` is a different problem — SQLite has no built-in
page encryption without a commercial extension or CGO, both of which this build
rules out — so what protects it remains what §6 said at the top: restrict who
can read the directory, and encrypt the archive. The reference mechanism does nothing for those: they
are generated by the hub, not supplied to it. Closing it needs a key the hub can
obtain at boot without a human present, which is an operator-supplied
passphrase, a TPM, or a mounted key file, and choosing between those is a design
question rather than an implementation gap. Until then the `tar czf` warning
above is the whole mitigation, and it is stated rather than implied.
