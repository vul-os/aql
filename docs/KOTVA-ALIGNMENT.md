...
2. **Replay defence is a different mechanism, not the same one.** Aql uses a durable single-use
   ...
   `controller/internal/command/command.go:119-134`). KOTVA gives content-address dedupe:
...
|---|---|---|---|
| **ADAPT-2** | Declare all four fields; per-direction where asymmetric (`:509`) | **Satisfied (2026-07-27)** | Declared in `hub/internal/channels/disclosure.go` for all four rails, per-direction where asymmetric (WhatsApp: free inbound, metered outbound, template-walled outside the window). Served unauthenticated at `GET /v1/rails/disclosure` so the fields can be compared *before* choosing a rail, which is what §26.3 is for. In Go rather than prose deliberately: a table nobody revisits is the failure mode, so a test asserts every registered rail declares. |
| **ADAPT-3** | Template-walled outbound-cold presented as a functional wall, never a price tier (`:510`) | **Not satisfied; latent** | Nothing models a service window or a template anywhere. Masked today because every outbound send is a reply to a verified inbound webhook. **But** `proto/events.md` specifies the intercom feature (`button` → *"the hub notifies the resident's chat — \"Someone is at the gate. Reply OPEN.\""*), and `ARCHITECTURE.md` lists it as "protocol supports now, ship later". That is a **hub-initiated cold send** — exactly §26.4.1's wall (`26-legacy-adapters.md:192-202`). Still unimplemented, but the reason has changed and this row said the old one for months: it claimed controller `event` uplinks were "signature-verified and then only logged", which stopped being true at migration 0019 — they are stored in `controller_events` and access-outcome kinds append to the hash-chained `access_logs`. What is unimplemented is the `button` KIND itself: `proto/events.md`'s Kinds table now marks it **reserved — no button input exists**, and `eventKinds.test.ts` fails if anything starts emitting it. The wall is therefore still ahead of the feature, which is the right order. |
...
| **ADAPT-11** | A free adapter still populates the exposure field; "free" never reads as "private" (`:518`) | **Satisfied (2026-07-27)** | Every rail's `Free` price carries an exposure naming the platform, and `TestNoRailClaimsNobodySeesPlaintext` fails any entry that says "nobody"/"only you"/"end-to-end" or omits the platform. The response's `note` states §26.5.1 outright: self-hosting removes the middleman operator, not the platform. |
