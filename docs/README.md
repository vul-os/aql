...
| [`GRANT-REVOCATION.md`](GRANT-REVOCATION.md) | Revoking an offline grant before its TTL expires, which `proto/grants.md` leaves as an open question. **Designed, not built.** A signed deny-list the controller caches while online and consults while offline; absence is never denial, so it cannot strand anyone, and a monotonic `seq` is what stops an attacker replaying an emptier list. |
