 ...
 *
 *   node scripts/check-site.mjs                 # serves site/ itself
 *   node scripts/check-site.mjs --selftest      # prove the checks can fail
 */
...
async function main() {
  if (!existsSync(join(SITE, 'index.html'))) {
    console.error(`check-site: no site/index.html under ${SITE}`);
    process.exit(2);
  }
  ...
  try {
    if (process.argv.includes('--selftest')) {
      console.log('check-site self-test — each invariant is broken on purpose:\n');
