#!/usr/bin/env bash
# =============================================================================
# scripts/verify.sh — verify a downloaded AQL release artifact, fail-closed.
#
...
#     name as a regex — every "." in "aql-hub_v9.9.9_linux_amd64.tar.gz" is a wildcard
...
# USAGE
# ─────
#   scripts/verify.sh --tag v0.1.0 aql-hub_v0.1.0_linux_amd64.tar.gz
#   scripts/verify.sh --tag v0.1.0 --attest aql-hub_v0.1.0_linux_amd64.tar.gz
#   scripts/verify.sh --repo vul-os/aql --tag v0.1.0 --out ~/Downloads ASSET
...
trap cleanup EXIT
TMPDIR_V="$(mktemp -d "${TMPDIR:-/tmp}/aql-verify.XXXXXX")"
...
ART   = "aql-hub_v9.9.9_linux_amd64.tar.gz"
GOOD  = b"aql-hub-dist-bytes\n" * 64
...
trap_sums = (line(ART + ".sig", OTHER) +
             line("aql-hub_v9X9X9_linux_amd64.tar.gz", OTHER)).encode()
...
run_selftest() {
  ...
  local art="aql-hub_v9.9.9_linux_amd64.tar.gz"
  ...
  expect_exit "--attest but no gh installed"  "$E_ATTEST"         yes -- env PATH="$fakebin" "$bash_abs" "$SELF_PATH" --out "$outd" --attest --repo vul-os/aql --base-url "$base/good" "$art"
