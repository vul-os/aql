...
# The !unix half of internal/recording. darwin IS unix, so diskfree_other.go
# never compiles here and a type error in it passed all nineteen gates —
# the same blindness the controller's ble/gpio files had, one module over.
# Typecheck only: nothing runs a Windows hub, but the file should still be a
# file that compiles.
run      "go vet (windows)"           hub env CGO_ENABLED=0 GOOS=windows go vet ./...
