...
| Layer | Status |
| --- | --- |
| Hub (`hub/`) — open path, console, API, device hub, audit | **Built.** 129 HTTP routes, 1,213 Go test functions green across 18 packages |
...
| Controller agent (`controller/`) — pairing, signed commands, grants, events | **Built.** 171 Go test functions green. GPIO relay driver and BLE radio are **not** |
...
│   └── migrations/   # SQLite schema, clean folded baseline (27 migrations, 51 tables)
...
Backed by **75 conformance vectors** across seven fixture files, and a `verify.mjs`
self-checker that independently re-canonicalizes, re-signs and re-evaluates each one —
**110 checks**, because multi-step transcripts contribute more than one. Both the hub and
